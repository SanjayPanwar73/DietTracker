...
const FoodLog = () => {
  ...
  const getMealStyle = (type) => {
    ...
    if (t.includes("breakfast")) return { icon: <Coffee className="w-4 h-4" />, bg: "bg-orange-100 text-orange-700" };
    if (t.includes("lunch"))     return { icon: <Sun className="w-4 h-4" />,    bg: "bg-yellow-100 text-yellow-700" };
    if (t.includes("dinner"))    return { icon: <Moon className="w-4 h-4" />,   bg: "bg-indigo-100 text-indigo-700" };
    if (t.includes("snack"))     return { icon: <Zap className="w-4 h-4" />,    bg: "bg-pink-100 text-pink-700" };
          ...
          <div className="flex items-center gap-3 flex-wrap justify-center">
            ...
            {isSameDay(selectedDate, new Date()) && (
              <button
                ...
                className="flex items-center gap-2 bg-green-600 text-white px-5 py-2.5 rounded-xl font-medium shadow-md shadow-green-200 hover:bg-green-700 transition active:scale-95"
              >
                ...
                return (
                  <div key={food._id} className="p-5 flex items-center justify-between hover:bg-gray-50 transition group">
                    <div className="flex items-center gap-4">
                      <div className={`p-2.5 rounded-lg ${mealStyle.bg}`}>{mealStyle.icon}</div>
                      <div>
            ...
            <div className="p-12 text-center flex flex-col items-center">
              ...
              {isSameDay(selectedDate, new Date()) && (
                <div className="flex gap-3">
                  <button onClick={() => navigate("/photo-log")} className="flex items-center gap-2 bg-emerald-50 text-emerald-700 border border-emerald-200 px-4 py-2.5 rounded-xl font-medium hover:bg-emerald-100 transition">
                    <Camera className="w-4 h-4" /> AI Photo Log
                  </button>
                  <button onClick={() => navigate("/add-food")} className="flex items-center gap-2 bg-green-600 text-white px-4 py-2.5 rounded-xl font-medium hover:bg-green-700 transition">
