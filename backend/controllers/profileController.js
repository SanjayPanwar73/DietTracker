...
exports.createProfile = async (req, res) => {
    ...
    try {
        ...
        else if (gender === "female") { bmr = 447.593 + (9.247 * weight) + (3.098 * height) - (4.330 * age); }
        else if (gender === "other") { 
            // For "other" gender, use average of male and female BMR
            const maleBmr = 88.362 + (13.397 * weight) + (4.799 * height) - (5.677 * age);
            const femaleBmr = 447.593 + (9.247 * weight) + (3.098 * height) - (4.330 * age);
            bmr = (maleBmr + femaleBmr) / 2;
        }
        else { return res.status(400).json({ success: false, message: "Invalid gender" }); }
        let tdee;
        switch (activityLevel) {
            ...
            case "super active": tdee = bmr * 2.0; break;
            default: throw new Error("Invalid activity level");
        }
        let calorieRequirement;
        if (goal === "weight loss") { calorieRequirement = tdee - 500; }
        else if (goal === "weight gain") { calorieRequirement = tdee + 500; }
        else { calorieRequirement = tdee; }
        const profile = new Profile({ user, height, weight, age, activityLevel, gender, goal, calorieRequirement });
        await profile.save();
...
exports.updateProfile = async (req, res) => {
    ...
    try {
        ...
        const profile = await Profile.findOneAndUpdate({ user }, { height, weight, age, activityLevel, gender, goal, calorieRequirement }, { new: true });
        if (!profile) { return res.status(404).json({ success: false, message: "Profile not found" }); }
